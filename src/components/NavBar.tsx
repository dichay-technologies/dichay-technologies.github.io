...
export function NavBar({ routes, location }: { routes: { path: string; name: string; element: JSX.Element; icon?:string }[]; location: Location; }) {
	return <nav className="z-50 w-full py-4 px-12 bg-background-600 h-min flex flex-row justify-between items-center text-white sticky top-0">
		<div className="">
		...
		<div className="flex md:hidden flex-row gap-4 text-xl font-bold items-baseline h-full">{routes.map(r => (
